...
bool SymbolCollector::handleMacroOccurrence(const IdentifierInfo *Name,
  ...
  if ((static_cast<unsigned>(Opts.RefFilter) & Roles) && !IsMainFileOnly &&
    ...
    // FIXME: Populate referrer information for macro references.
    MacroRefs[*ID].push_back({Loc, Roles, /*Referrer=*/nullptr});
...
void SymbolCollector::finish() {
  ...
  auto CollectRef = [&](SymbolID ID, const SymbolRef &LocAndRole,
    ...
    if (auto FileURI = GetURI(FileID)) {
      ...
      if (auto RefID = getSymbolID(LocAndRole.Referrer))
        R.Referrer = *RefID;
      Refs.insert(ID, R);
    }
  };
